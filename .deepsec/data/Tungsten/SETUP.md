# Agent setup for `Tungsten`

This is a deepsec scanning workspace. Project `Tungsten` was just registered
(target: `..`). Setup is incomplete — `data/Tungsten/INFO.md`
...
2. **Fill in `data/Tungsten/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id Tungsten
pnpm deepsec process --project-id Tungsten
```
